...
$(document).ready(function() {
	...
	$("#regenButton").click(regenerateMeals);
	$("#generateButton").click(function(){
		if(listOfMeals.length == 0)
			$('#centralSpinner').show();
		regenerateMeals();
	});
	$("#regenButtonFromChange").click(regenerateMeals);
	$("#newUserRegenButton").click(regenerateMeals);
	$("#errorRegenButton").click(regenerateMeals);
	$("#mealsRow").hide();
	...
	for(let i = 0; i < userNumOfMeals; i++){
		...
		$(`#meal_${i+1}_title`).click(function(){
		...
		$(`#checkbox${i+1}`).on('change', function(){ 
			var mealBody = document.getElementById(`card-body${i+1}`);
		   if(this.checked){
		        mealBody.style.display = "none";
		        swal({
	...
	function getMeal(){
		...
		if(mealsGenerated >= userNumOfMeals){
			...
			$('#mainRefresh').show();
			$('#centralSpinner').hide();
			$("#generateButton").html('Regenerate Plan');
	...
	function displayMeals(){
		...
		for(let i = 0; i < listOfMeals.length; i++){
			...
			document.getElementById(`expandButton${modified_i}`).style.textDecoration ="none";
			document.getElementById(`expandButton${modified_i}`).style.color ="white";
			...
			document.getElementById(`expandButtonMobile${modified_i}`).style.textDecoration ="none";
			document.getElementById(`expandButtonMobile${modified_i}`).style.color ="black";
	...
	function regenerateInidividualMeal(index){
		...
		if(w <= 1000){
			$(`#mobileSpinner${index+1}`).show(); 
		} 
		else {
			$(`#spinner${index+1}`).show(); 
			$(`#refresh${index+1}`).hide();
		}	
		...
		$.ajax(settings).done(async function (response) {
			...
			displayMeals();		
			if(w <= 1000){
				$(`#mobileSpinner${index+1}`).hide(); 
			} 
			else {
				$(`#spinner${index+1}`).hide(); 
				$(`#refresh${index+1}`).show();
			}	
